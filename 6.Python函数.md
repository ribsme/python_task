...
#### lambda函数是一个可以接收任意多个参数并且返回单个表达式值的匿名函数12。
...
#### 高阶函数是指可以接受一个或多个函数作为参数，并且可以返回一个函数作为结果的函数。高阶函数可以把函数作为参数传递给另外一个函数，也可以把函数作为另一个函数的返回结果。在函数式编程中，高阶函数是一个非常重要的概念，它可以帮助我们将代码写得更加简洁、灵活和可复用。例如，Array.prototype.map，Array.prototype.filter 和 Array.prototype.reduce 是语言中内置的一些高阶函数。
